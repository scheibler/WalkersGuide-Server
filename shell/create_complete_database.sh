...
result=$(psql -l | cut -d '|' -f1 | tr -d ' ' | grep "^$db_tmp_name$")
if [ ! -z "$result" ]; then
    # end all potential active connections to the tmp database
    psql -d postgres -c "select pg_terminate_backend(pid) from pg_stat_activity where datname = '$db_tmp_name';"
    # delete
    psql -d postgres -c "DROP DATABASE $db_tmp_name;"
    if [[ $? != 0 ]]; then
        exit 21
    fi
fi
...
# create new db
createdb -O $db_owner "$db_tmp_name"
if [[ $? != 0 ]]; then
    exit 1
fi
...
psql -d $db_tmp_name -c "CREATE EXTENSION postgis;"
if [[ $? != 0 ]]; then
    exit 1
fi
psql -d $db_tmp_name -c "CREATE EXTENSION hstore;"
if [[ $? != 0 ]]; then
    exit 1
fi
psql -d $db_tmp_name -c "CREATE EXTENSION pgrouting;"
if [[ $? != 0 ]]; then
    exit 1
fi
...
if [ "$( psql -A -t -d $db_active_name -c "$check_if_access_statistics_table_exists_command" )" = 't' ]; then
    pg_dump -d $db_active_name -t $db_access_statistics_table | psql -d $db_tmp_name
    echo -e "\nRescued access statistics table"
fi

# clean up new database
echo -e "\nanalyse database -- started at $(get_timestamp)"
psql -d $db_tmp_name -c "VACUUM ANALYZE;"
if [[ $? != 0 ]]; then
...
result=$(psql -l | cut -d '|' -f1 | tr -d ' ' | grep "^$db_active_name$")
if [ ! -z "$result" ]; then
    # end all potential active connections to the productive database
    psql -d postgres -c "select pg_terminate_backend(pid) from pg_stat_activity where datname = '$db_active_name';"
    # delete
    psql -d postgres -c "DROP DATABASE $db_active_name;"
    if [[ $? != 0 ]]; then
...
psql -d postgres -c "ALTER DATABASE $db_tmp_name RENAME TO $db_active_name;"
if [[ $? != 0 ]]; then
